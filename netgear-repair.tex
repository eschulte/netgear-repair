...
\begin{abstract}
  When a security researcher discovers a software exploit, she must
  decide whether to publish the exploit information, informing users
  and potential attackers, or to privately inform the software vendor,
  leaving users in the dark until the vendor takes action.  Using an
  alternative approach presented here, researchers can instead use
  newly discovered exploits to guide an automated repair process which
  is capable of patching exploits even without access to source code
  or special information from the software vendor.

  The repair process presented here is an extension of earlier work
  that developed automated software repair techniques using
  evolutionary computation.  Here, we adapt the technique to the
  special requirements of a NETGEAR router exploit, including
  modifications for stripped MIPS ELF files, removing the requirement
  for fault localization, and demonstrating that the algorithm can
  work without any pre-existing regression test suite.  Under this
  rigorous test scenario, 80\% of proposed repairs for the example
  exploit retained program functionality.

The scenario is illustrated by patching recently discovered exploits
in version 4 of NETGEAR's WNDR3700 wireless router---at the time of submission,
% \emph{before} NETGEAR
%publicly addressed the exploits---to the author's knowledge 
NETGEAR had not publicly released patches for these signficant
security exploits.  
%We discuss
...
\end{tikzpicture}
\caption{Mutation and Crossover operations in MIPS ELF files.  The
