...
#+begin_src ruby
  ...
  # Input: Interactive Evaluation, good-enough: ELF -> (list (ELF -> Fitness))
  ...
  do
    ...
    do
      if (random() < cross-rate)
        new <- crossover(tournament(pop, tournament-size, +),
                         tournament(pop, tournament-size, +))
      else
        new <- tournament(pop, tournament-size, +)
      end
      fitness <- 0
