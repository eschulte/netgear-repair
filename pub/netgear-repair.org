...
#+begin_src fundamental
  ...
  # Parameters: max-population-size, tournament-size
  # Output: Patched version of Program
...
selected for eviction using a negative tournament of size 2.

Every time an individual is selected 
...
tests fitness.  When any thread finds a repair termination of the
repair loop of the algorithm terminates globally and the candidate
repair is presented to the user (line XX of Listing [[#lazy-algorithm]]).
...
| # regression tests | evaluations per minute / 32 cores |
|--------------------+-----------------------------------|
|                  0 |                                   |
|                  1 |                                   |
|                  2 |                                   |
