...
only writes regression tests as needed and find that in 80% of cases
the NETGEAR WNDR3700 exploits may be repaired without having to write
/any/ regression tests.  We discuss the feasibility and utility of
this technique for the security community.
#+LaTeX: \end{abstract}
...
3. a modified version of evolutionary program repair which does not
   use fault localization (would have made this repair impossible)
4. a modified version of evolutionary program repair which removes the
   requirement for a regression test suite
5. an application of automated repair to a real-world un-patched
   security exploit resulting in
6. the first demonstration of multiple repair in a single evolutionary
   run.
...
** Lazy on demand Regression Testing
   ...
   :CUSTOM_ID: lazy
   :END:
We present a novel evolutionary program repair which does not require
a regression test suite.  We adopt the repair algorithm from
cite:forrest2009genetic but instead of assuming that a regression test
suite exists at the beginning of the algorithm, we only assume that a
single test case exists exercising the exploit.  High level pseudocode
for the repair algorithm is show in Figure [[lazy-algorithm]].
...
(see [[file:~/research/epr/topics/netgear-repair/NOTES::*evaluate%20lazy%20regression%20testing][evaluate lazy regression testing]])

Report timing results for different sizes of test suites.
...
* Related Work
** Security
- motivate the use case, questions of when to report
- move much of the intro to here
